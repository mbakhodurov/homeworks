...
func InventoryFilterModelToProto(from model.InventoryFilter) *inventory_v1.PartsFilter {
...
func ListPartInventoryResponseProtoToModel(from *inventory_v1.ListPartInventoryResponse) []model.Inventory {
	if from == nil {
	...
	for _, part := range from.Inventory {
		...
		part := model.Inventory{
			...
			CreatedAt:     *timestampToPtr(part.CreatedAt),
