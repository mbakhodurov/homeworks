...
func OrderModelToOpenApiModelOrder(from model.Order) order_v1.OrderDto {
	dto := order_v1.OrderDto{
		OrderUUID:  from.OrderUUID,
		UserUUID:   from.UserUUID,
		PartUuids:  from.PartUUID,
	...
	if from.UpdatedAt != nil {
		dto.UpdatedAt = order_v1.NewOptNilDateTime(*from.UpdatedAt)
	}
...
func PaymentMethodDtoToString(domainMethod model.PaymentMethod) string {
	switch domainMethod {
	case model.PaymentMethodCard:
		return Card
	case model.PaymentMethodSBP:
