...
import (
	...
	inventoryClient "github.com/mbakhodurov/homeworks/week6/order/internal/client/grpc/inventory/v1"
	paymentClient "github.com/mbakhodurov/homeworks/week6/order/internal/client/grpc/payment/v1"
	"github.com/mbakhodurov/homeworks/week6/order/internal/converter/kafka"
	"github.com/mbakhodurov/homeworks/week6/order/internal/converter/kafka/decoder"

	serviceHandler "github.com/mbakhodurov/homeworks/week6/order/internal/api/order/v1"
	"github.com/mbakhodurov/homeworks/week6/order/internal/config"

	repoOrderInterface "github.com/mbakhodurov/homeworks/week6/order/internal/repository"
	repo "github.com/mbakhodurov/homeworks/week6/order/internal/repository/order"
	serviceOrderInterface "github.com/mbakhodurov/homeworks/week6/order/internal/service"
	orderconsumer "github.com/mbakhodurov/homeworks/week6/order/internal/service/consumer/order_consumer"
	service "github.com/mbakhodurov/homeworks/week6/order/internal/service/order"
	"github.com/mbakhodurov/homeworks/week6/platform/pkg/closer"

	wrappedKafka "github.com/mbakhodurov/homeworks/week6/platform/pkg/kafka"
	wrappedKafkaProducer "github.com/mbakhodurov/homeworks/week6/platform/pkg/kafka/producer"
	"github.com/mbakhodurov/homeworks/week6/platform/pkg/logger"

	kafkaMiddleware "github.com/mbakhodurov/homeworks/week6/platform/pkg/middleware/kafka"
	...
	order_v1 "github.com/mbakhodurov/homeworks/week6/shared/pkg/openapi/order/v1"
	...
	orderProducer "github.com/mbakhodurov/homeworks/week6/order/internal/service/producer/order_producer"
	wrappedKafkaConsumer "github.com/mbakhodurov/homeworks/week6/platform/pkg/kafka/consumer"
)
...
func (d *diContainer) IAMGRPCConn(_ context.Context) *grpc.ClientConn {
	if d.iamGRPCConn == nil {
		conn, err := grpc.NewClient(
			config.ApppConfig().IAMClient.Address(),
...
func (di *diContainer) OrderService(ctx context.Context) serviceOrderInterface.OrderService {
	if di.orderService == nil {
		di.orderService = service.NewService(
			di.OrderRepo(ctx),
...
func (di *diContainer) OrderRepo(ctx context.Context) repoOrderInterface.OrderRepository {
	if di.orderRepo == nil {
		di.orderRepo = repo.NewRepository(di.SqlDB(ctx))
	}
	return di.orderRepo
}
